...
const News = (props) => {
  ...
  return (
    <div className={styles.news}>
      ...
      <hr/>
      {props.news.length == 0 &&
      <h1 style={{marginBottom:400}}>Новостей нет!</h1>
       }
      {props.news.map(e => {
        return (
          <div key={e.id} className={styles.newsBox}>
            ...
            <div className={styles.imgBox}>
              <img className={styles.imgNews} alt="фотография" src={'./img/' + e.img} />
            </div>
            <p className={styles.newsDate}>{moment(e.date).utc().format('DD.MM.YYYY')}</p>
